...
export const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <article 
      className="bg-white dark:bg-slate-800 rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-slate-700 flex flex-col overflow-hidden group"
      itemScope 
      ...
      <div className="p-6 flex-1">
        <div className="flex justify-between items-start mb-4">
          ...
          >
            ...
            <meta itemProp="availability" content={product.stock > 0 ? "http://schema.org/InStock" : "http://schema.org/OutOfStock"} />
            In Stock: {product.stock}
          </span>
        </div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1" itemProp="name">{product.title}</h3>
